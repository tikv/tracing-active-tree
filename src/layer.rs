...
use crate::tree::formating::FormatTreeTo;
...
impl CurrentStacksLayer {
    ...
    pub fn fmt_bytes(&self) -> Vec<u8> {
        self.fmt_bytes_with(|tree, buf| tree.traverse_with(FormatTreeTo::unicode(buf)).unwrap())
    }

    pub fn fmt_bytes_with(&self, formatter: impl Fn(&Tree, &mut Cursor<Vec<u8>>)) -> Vec<u8> {
        use std::io::Write;
        ...
        for ent in self.roots.iter() {
            ...
            match tree.try_lock() {
                Ok(tree) => {
                    formatter(&tree, &mut res);
                }
                Err(_) => {
                    writeln!(res, "[LOCKED] [span_id={}]", id.into_u64()).unwrap();
                }
            }
        }
        res.into_inner()
    ...
    S: Subscriber + for<'a> LookupSpan<'a>,
{
                ...
                None => {
                    let ptr = match self.roots.entry(id.clone()) {
                        ...
                        Entry::Vacant(v) => {
                            ...
                            tracing::debug!(id = ?id, "inserting new root span.");
                            v.insert(ptr.clone());
                            ptr
                        }
                    };
    ...
    fn on_close(&self, id: span::Id, _ctx: tracing_subscriber::layer::Context<'_, S>) {
        tracing::debug!(id = ?id, "span closed.");
        CURRENT_FRAME_STACK.with(|cell| {
            let c = cell.borrow();
