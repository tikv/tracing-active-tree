...
use self::formating::FormatTreeTo;
...
impl Tree {
    ...
    pub fn fmt_bytes(&self) -> Vec<u8> {
        let mut res = vec![];
        self.traverse_with(FormatTreeTo::unicode(&mut res))
...
pub mod formating {
    ...
    pub struct FormatFlat<W> {
        forked: SmallVec<[(usize, u32); 8]>,
        current_idx: usize,
        output: W,
    }
    ...
    impl<W: Write> TreeVisit for FormatFlat<W> {
        ...
        fn enter(
            ...
            match self.forked.last() {
                Some(&(last_idx, _)) if self.current_idx == last_idx => {
                    self.forked.last_mut().unwrap().1 += 1;
                }
                _ if tree[node].next_sibling().is_some() => {
                    self.forked.push((self.current_idx, 1));
                }
                _ => {}
            }

            // Skip all non-forked frames, once the spanstack has been forked,
            // we print which "possibility" this frame stands for.
            for (idx, &(_, i)) in self.forked.iter().enumerate() {
                if idx == 0 {
                    write!(self.output, " &{i}")?;
                } else {
                    write!(self.output, ",{i}")?;
